...
(function () {
  function r(e, n, t) {
    ...
    function seeMore() {
      // pour afficher le div qui contient les infos sur les dames chinoises
      ...
      if (x.style.display === "none") {
        // si le div est masqué, on affiche le div et on change le label du botuon
        x.style.display = "block";
        y.innerHTML = "Voir moins";
      } else {
        // sinon, on masque le div et on change le label du bouton
    ...
    function updateChoices() {
      // fonction déclenchée quand on change le nombre de joueurs
      ...
      for (var i = 2; i <= 6; i++) {
        // pour tous les champs de nom de joueur à partir du 2e
      ...
      for (var i in checkNewGameForm) {
        // pour chaque fonction de vérification su rl eocntenu du formulaire
        if (document.getElementById(i).value != '' && parseInt(i[6]) <= mode) {
          // on ne vérifie que si quelque chose a été saisi et si le joueur va jouer
    ...
    function updateColors() {
      // met à jour l'affichage des couleurs à droite du nom des joueurs
      var list = document.getElementById("mode"); // on récupère le menu déroulant
      ...
      for (var n = 1; n <= mode; n++) {
        // pour chaque joueur qui joue
        var code = '';
        for (var i = 0, max = colors[n - 1].length; i < max; i++) {
          // pour chaque couleur on affiche les images correspondanes
      ...
      for (var n = mode + 1; n <= 6; n++) {
        // on masque les joueurs qui ne jouent pas
    ...
    function deactivateTooltips() {
      ...
      var tooltips = document.querySelectorAll('.tooltip'),
          // on sélectionne tous les éléments de la classe tooltip
    ...
    function getTooltip(elements) {
      // on récupére la "tooltip" qui correspond à l'input
    ...
    function check_player(id) {
      ...
      if (regex.test(player.value) || player.value === '') {
        // si la regex est validée ou le champs et vide
        if (typeof id[6] === 'undefined') {
          // pour le champs "nom" du formulaire pour rejoindre une partie
          player.className = 'correct'; // on valide
          tooltip.style.display = 'none'; // on cache le tooltip
          return true;
        } else {
          // sinon pour le formulaire de création on vérifie si le champs et vide, que le nom n'est pas pris et que je joueur va jouer
          if (player.value === '' || id[6] !== '1' && document.getElementById("ordi" + id[6]).checked || // A ENLEVER JOUEUR 1 PAS IA
          (player === player1 || player.value != player1.value) && (player === player2 || player.value != player2.value || player2.style.display == 'none') && (player === player3 || player.value != player3.value || player3.style.display == 'none') && (player === player4 || player.value != player4.value || player4.style.display == 'none') && (player === player5 || player.value != player5.value || player5.style.display == 'none') && (player === player6 || player.value != player6.value || player6.style.display == 'none')) {
            player.className = 'correct'; // on valide
            tooltip.style.display = 'none'; // on cache le tooltip
            return true;
          } else {
    ...
    checkJoinGameForm['roomID'] = function () {
      // fonction pour vérifier le numéro de la salle de jeu saisi pour rejoindre une partie
      ...
      if (regex.test(input.value) && input.value <= 100000) {
        // si la regex est validée et que la valeur et inférieure à 100000
        input.className = 'correct'; // on valide
        tooltip.style.display = 'none'; // on cache le tooltip
        return true;
      } else {
        // si la regex n'est pas validée ou que la valeur est trop grande
    ...
    function disablePlayer(n) {
      // fonction pour masquer les champs d'un joueur inactif
      ...
      if (checkbox.checked) {
        ...
        // si la checkbox n'est aps cochée
    ...
    var newGameForm = document.getElementById('newGameForm'),
        // on récupère le formulaire de création de partie
    inputs = document.querySelectorAll('input[type=text]'); // on récupère tous les champs de texte du formulaire
    for (var i = 0; i <= 5; i++) {
      // pour les joueurs 1 à 6
      inputs[i].addEventListener('keyup', function (event) {
        // on lie la fonction de vérification à l'évènement
    ...
    newGameForm.addEventListener('submit', function (event) {
      // on lie une fonction à l'évènement submit pour une nouvelle partie
      var result = true;
      ...
      for (var i in checkNewGameForm) {
        // pour chaque fonction de vérification
    ...
    var boxes = document.querySelectorAll('input[type=checkbox]'),
        // on séléctionne tous les inputs de type checkbox
    boxesLength = boxes.length;
    for (var i = 0; i < boxesLength; i++) {
      // pour chaque checkbox
      boxes[i].addEventListener('click', function (event) {
        // on lie la fonction de vérification à l'event "click"
    ...
    for (var i = 2; i <= 6; i++) {
      // pour tous les joueurs sauf le premier
    ...
    Home.seeMore();
  }, { "./home.js": 1 }], 3: [function (require, module, exports) {
    (function (exports) {
...
//# sourceMappingURL=/home/khornya/L2INF/Projet/dames-chinoises/node_modules/.cache/ava/ebf48e290aff93caff12574963d47a63.js.map